...
class PointResetPublisher(Node):
    def __init__(self):
        super().__init__('point_reset_publisher')
        self.publisher_ = self.create_publisher(Point, 'reset_point_topic', 10)
    ...
    def timer_callback(self):
        ...
        # Keep publishing the message for a few cycles
        if self.i > 5:
            self.destroy_node()

def main(args=None):
    rclpy.init(args=args)
    point_reset_publisher = PointResetPublisher()
    rclpy.spin(point_reset_publisher)

    # Shutdown
    point_reset_publisher.destroy_node()
    rclpy.shutdown()
...
#             {'x': 1.016, 'y': 2.335, **quaternion_left},  # Menghadap ke kanan
#             {'x': 0.072, 'y': 2.398, **quaternion_right}  # Menghadap ke kanan
#         ]
#         self.return_waypoints = [
#             {'x': 1.016, 'y': 2.335, **quaternion_down},  # Menghadap ke kanan
